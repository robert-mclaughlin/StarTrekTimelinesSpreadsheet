...
export class ReplicatorDialog extends React.Component {
    ...
    _reloadItems() {
        if (this.state.fuelconfig === 'extraSchematics') {
            ...
            let fuellist = STTApi.playerData.character.items.filter(item => equipmentAlreadyOnCrew.has(item.archetype_id) && (item.quantity === 1)  && (item.rarity > 1));
            this.setState({fuellist});
    ...
    render() {
        ...
        >
        <div style={{ minWidth: '800px', color: currentTheme.semanticColors.bodyText, backgroundColor: currentTheme.semanticColors.bodyBackground }}>
            {!this.state.canBeReplicated && <p><b>This item cannot be replicated!</b></p>}
            <div style={{color: 'red'}}>The actual replicator functionality is not implemented yet. Feel free to browse the tables on the left for inspiration though.</div>

            <div style={{ display: 'grid', gridTemplateColumns: '9fr 7fr', gridGap: '6px', gridTemplateAreas: `'fuelconfig fueltank' 'fuellist fueltank' 'fuellist details'` }}>
                <div style={{ gridArea: 'fuelconfig', display: 'grid', gridTemplateColumns: '2fr 1fr' }}>
                    <Dropdown
                        selectedKey={this.state.fuelconfig}
                        onChange={(evt, item) => { this.setState({ fuelconfig: item.key }, () => { this._reloadItems(); }); }}
                    ...
                    <ReactTable
                        ...
                        columns={ [
                            ...
                            },
                            {
                                ...
                                sortable: false,
                                resizable: true,
                                Cell: (row) => <button className="ui icon button" onClick={() => this._removeFromTank(row.original)}><i className="icon close"></i></button>
                            }]}
                        ...
                        style={{ height: '280px' }}
                    />
                    <ProgressIndicator description={`Fuel: ${this.state.fuelTankValue} of ${this.state.fuelCost}`} percentComplete={this.state.fuelTankValue * 100 / this.state.fuelCost} />
                </div>
            ...
            <DialogFooter>
                <PrimaryButton onClick={this._closeDialog} text='Replicate' disabled={this.state.canBeReplicated && (this.state.fuelTankValue < this.state.fuelCost)} />
