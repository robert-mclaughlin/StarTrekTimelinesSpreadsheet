...
class Recommendation extends React.Component {
    ...
    render() {
        return <div style={{ display: 'grid', gridTemplateColumns: 'minmax(min-content,50px) auto', gridTemplateAreas: `'icon description'`, marginBottom: '10px' }}>
            <div style={{ gridArea: 'icon' }}>
                <i className={"huge icon " + this.props.icon || Priority.CHECK}></i>
            </div>
    ...
    constructor(props) {
        ...
        if (replicator_uses_left === 0) {
            ...
            recommendations.push({
                ...
                content: <p style={{ margin: '0' }}>You have {replicator_uses_left} replicator uses left for today. See the 'Needed Equipment' tab for recommendations on what to spend them on.</p>
            });
        }
        ...
        if (STTApi.playerData.character.cadet_tickets.current === 0) {
            recommendations.push({
                ...
                icon: Priority.CHECK,
                content: <p style={{ margin: '0' }}>You used all cadet tickets ({STTApi.playerData.character.cadet_tickets.max}) for today's {cadet.title}.</p>
            });
        } else {
            recommendations.push({
                ...
                content: <div>
                    <p style={{ margin: '0' }}>You have {STTApi.playerData.character.cadet_tickets.current} cadet tickets left for today's {cadet.title}; it ends in {formatTimeSeconds(STTApi.playerData.character.cadet_schedule.ends_in)}.</p>
        ...
        if (itemCount > STTApi.playerData.character.item_limit - 5) {
            recommendations.push({
                ...
                content: <div>
                    <p style={{ margin: '0' }}>You have {STTApi.playerData.character.items.length} types of items in your inventory out of a maximum of {STTApi.playerData.character.item_limit}; the game is randomly dismissing items.</p>
                    <p style={{ margin: '0' }}>Get rid of some items by equipping, or throwing them in the replicator. See the 'Needed Equipment' tab for recommendations on what equipment you may no longer need.</p>
                </div>
            });
            ...
            recommendations.push({
                ...
                content: <div>
                    <p style={{ margin: '0' }}>You have {STTApi.playerData.character.items.length} types of items in your inventory out of a maximum of {STTApi.playerData.character.item_limit}.</p>
                    <p style={{ margin: '0' }}>Consider getting rid of some items by equipping, or throwing them in the replicator. See the 'Needed Equipment' tab for recommendations on what equipment you may no longer need.</p>
                </div>
            });
        } else {
            recommendations.push({
                ...
                icon: Priority.INFO,
                content: <p style={{ margin: '0' }}>All good. You have {STTApi.playerData.character.items.length} types of items in your inventory out of a maximum of {STTApi.playerData.character.item_limit}.</p>
            });
        }
        ...
        if (overflowingItems.length > 0) {
            recommendations.push({
                ...
                icon: Priority.EXCLAMATION,
                content: <div>
                    <p style={{ margin: '0' }}>Some of your items are overflowing; the item quantity is capped at 32768 by the game, and any extras are lost:</p>
                    <div style={{ display: 'flex' }}>
                        {overflowingItems.map((item, idx) => <span style={{ display: 'contents' }} key={idx}><ItemDisplay src={item.iconUrl} size={24} maxRarity={item.rarity} rarity={item.rarity} /> {item.name} ({item.quantity}) </span>)}
                    </div>
                </div>
            });
        }
                ...
                if (voyage.state === "recalled") {
                    if (voyage.recall_time_left > 0) {
                        recommendations.push({
                            ...
                            icon: Priority.CHECK,
                            content: <p style={{ margin: '0' }}>Voyage has lasted for {formatTimeSeconds(voyage.voyage_duration)} and it's currently returning ({formatTimeSeconds(voyage.recall_time_left)} left).</p>
                        });
                    } else {
                        recommendations.push({
                    ...
                    recommendations.push({
                        title: `Voyage failed`,
                        icon: Priority.EXCLAMATIONRED,
                        content: <p style={{ margin: '0' }}>Voyage has run out of antimatter after {formatTimeSeconds(voyage.voyage_duration)} and it's waiting to be abandoned or replenished.</p>
                    });
                    ...
                    recommendations.push({
                        ...
                        icon: Priority.CHECK,
                        content: <p style={{ margin: '0' }}>Voyage has been ongoing for {formatTimeSeconds(voyage.voyage_duration)} (new dilemma in {formatTimeSeconds(voyage.seconds_between_dilemmas - voyage.seconds_since_last_dilemma)}).</p>
                    });
                }
            });
        }
        ...
        loadGauntlet().then((gauntlet) => {
            ...
            if (gauntlet.state === 'NONE') {
                newRecommendation = {
                    ...
                    icon: Priority.EXCLAMATION,
                    content: <p style={{ margin: '0' }}>You're not currently in a gauntlet; join one in the 'Gauntlet' tab, you're missing out on rewards. Next gauntlet starts in {formatTimeSeconds(gauntlet.seconds_to_join)}.</p>
                };
                ...
                newRecommendation = {
                    ...
                    icon: Priority.INFO,
                    content: <p style={{ margin: '0' }}>The gauntlet ends in {formatTimeSeconds(gauntlet.seconds_to_end)}, next crew refresh in {formatTimeSeconds(gauntlet.seconds_to_next_crew_refresh)}.</p>
                };
    ...
    render() {
        ...
        return <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'min-content auto', gridTemplateAreas: `'image description'` }}>
                ...
                <div style={{ gridArea: 'description' }}>
                    <a style={{ float: 'right' }} href="https://www.patreon.com/bePatron?u=10555637" target='_blank' data-patreon-widget-type="become-patron-button"><img src='https://c5.patreon.com/external/logo/become_a_patron_button.png'/></a>

                    <h3>Welcome, {STTApi.playerData.character.display_name}!</h3>
                    <p>DBID {STTApi.playerData.dbid}</p>
                    <p>Location {STTApi.playerData.character.navmap.places.find((place) => place.symbol === STTApi.playerData.character.location.place).display_name}</p>
                    ...
                    {STTApi.playerData.motd && <p style={{ fontSize:'0.9em' }} dangerouslySetInnerHTML={{ __html: STTApi.playerData.motd.text.trim().replace(/(?:\r\n|\r|\n)/g, '<br />') }} />}
                </div>
            </div>

            <div>
                {recommendations.map((r, idx) => <Recommendation key={idx} title={r.title} icon={r.icon}>{r.content}</Recommendation>)}
            </div>
        </div>;
    }
}